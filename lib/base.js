...
class Base {
  ...
  // confirmRenameByTelemetry(uid, reportedName): lost-ACK resilience, mirrors
  // the firmware's updateNodeState() helper. A set_name ACK is a single
  // unacknowledged packet — if it's lost over RF the rename still SUCCEEDED on
  // the collar, but the command would sit AWAITING_ACK until it ages out. Any
  // later packet that reports the collar's current name lets us confirm a
  // pending rename to that UID whose requested name now matches.
  _confirmRenameByTelemetry(uid, reportedName) {
    if (!uid || !reportedName) return false;
    let any = false;
    for (const c of this.queue) {
      if (c.targetId !== uid) continue;
      if (![P.STATUS.QUEUED, P.STATUS.SENDING, P.STATUS.AWAITING_ACK].includes(c.status)) continue;
      if (!c.obj || c.obj.cmd !== "set_name") continue;
      if (c.obj.name && c.obj.name === reportedName) {
        this.log(`[CMD] ${c.msgId} rename→'${c.obj.name}' confirmed by reported name (implicit DELIVERED — ACK lost)`);
        this._setStatus(c, P.STATUS.DELIVERED);
        any = true;
      }
    }
  ...
  _updateNodeState(p) {
    ...
    // V3.6.6: telemetry/pong/status (no "ack") reporting the current name
    // confirms a pending rename whose requested name now matches.
    if (!p.ack) this._confirmRenameByTelemetry(uid, name);
