...
class Channel {
  ...
  constructor(clock, opts = {}) {
    ...
    // Optional deterministic drop hook: dropFilter(senderId, obj, receiverId)
    // → true drops that specific packet for that receiver. Lets a scenario
    // lose EXACTLY one packet type (e.g. only the set_name ACK) without
    // relying on probabilistic lossRate.
    this.dropFilter = opts.dropFilter || null;
    this.radios = [];
  ...
  transmit(senderId, obj) {
    ...
    this.clock.at(this.airtimeMs + this.propagationMs, () => {
      for (const r of this.radios) {
        ...
        if (this.dropFilter && this.dropFilter(senderId, obj, r.id)) {
          this.stats.lost++;
          this.log(`DROP ${pad(r.id)}  (filtered)        ${pkt}`);
          continue;
        }
